...
import com.mayanshe.nosocomiumonline.patient.requests.ModifyMediaInfoRequest;
...
public class MediaController {
    ...
    public MediaDto upload(
            @Parameter(
                    ...
                    example = "images",
                    required = true,
                    schema = @Schema(allowableValues = {"images", "audios", "videos", "documents", "archives"})
            )
            @PathVariable("mediaType") String mediaType,
            @Parameter(
                    description = "文件的业务分类 (可选)",
                    example = "avatar"
            )
            @RequestParam(value = "kind", required = false) String kind,
            @Parameter(
                    description = "待上传的文件文件",
                    required = true
            )
